import pandas as pd
import numpy as np
from sklearn.model_selection import train_test_split, cross_val_score
from sklearn.metrics import accuracy_score, f1_score, classification_report
from sklearn.ensemble import AdaBoostClassifier, GradientBoostingClassifier, ExtraTreesClassifier, VotingClassifier
from sklearn.neural_network import MLPClassifier
from sklearn.linear_model import LogisticRegression
from sklearn.svm import SVC
from sklearn.tree import DecisionTreeClassifier
from sklearn.ensemble import RandomForestClassifier
from xgboost import XGBClassifier
from sklearn.preprocessing import LabelEncoder
import joblib
import os

def create_ensemble_6():
    """
    Ensemble 6: Adaptive Boosting + Gradient Boosting + Extra Trees
    Uses different boosting and tree-based ensemble methods
    """
    # Base estimators with different characteristics
    ada = AdaBoostClassifier(
        estimator=DecisionTreeClassifier(max_depth=3),
        n_estimators=100,
        learning_rate=0.8,
        random_state=42
    )
    
    gb = GradientBoostingClassifier(
        n_estimators=100,
        learning_rate=0.1,
        max_depth=6,
        random_state=42
    )
    
    et = ExtraTreesClassifier(
        n_estimators=200,
        max_depth=10,
        min_samples_split=5,
        random_state=42
    )
    
    # Create voting classifier with adaptive weights
    return VotingClassifier(
        estimators=[('ada', ada), ('gb', gb), ('et', et)],
        voting='soft',
        weights=[1.2, 1.0, 0.8]  # Give more weight to AdaBoost and GradientBoosting
    )

def create_ensemble_7():
    """
    Ensemble 7: Blending with Meta-Learning
    Uses a two-level approach with neural network as meta-learner
    """
    # Level 1: Base models
    base_models = [
        ('rf', RandomForestClassifier(n_estimators=200, random_state=42)),
        ('xgb', XGBClassifier(n_estimators=200, random_state=42, use_label_encoder=False, eval_metric='mlogloss')),
        ('svm', SVC(probability=True, kernel='rbf', C=1.0)),
        ('nn', MLPClassifier(hidden_layer_sizes=(100, 50), max_iter=1000, random_state=42)),
        ('lr', LogisticRegression(max_iter=1000, random_state=42))
    ]
    
    # Level 2: Meta-learner (Neural Network)
    meta_learner = MLPClassifier(
        hidden_layer_sizes=(50, 25),
        max_iter=1000,
        random_state=42
    )
    
    # Create stacking classifier with neural network meta-learner
    from sklearn.ensemble import StackingClassifier
    return StackingClassifier(
        estimators=base_models,
        final_estimator=meta_learner,
        cv=5,
        stack_method='predict_proba'  # Use probability predictions
    )

def create_ensemble_8():
    """
    Ensemble 8: Weighted Average with Cross-Validation Optimization
    Uses cross-validation to find optimal weights for model combination
    """
    # Individual models
    models = {
        'rf': RandomForestClassifier(n_estimators=200, random_state=42),
        'xgb': XGBClassifier(n_estimators=200, random_state=42, use_label_encoder=False, eval_metric='mlogloss'),
        'svm': SVC(probability=True, kernel='rbf', C=1.0),
        'nn': MLPClassifier(hidden_layer_sizes=(100, 50), max_iter=1000, random_state=42),
        'gb': GradientBoostingClassifier(n_estimators=100, random_state=42)
    }
    
    # Use a simpler approach with VotingClassifier instead of custom class
    return VotingClassifier(
        estimators=list(models.items()),
        voting='soft',
        weights=[1.0, 1.2, 0.8, 0.9, 1.1]
    )

def train_and_evaluate(model, X_train, X_test, y_train, y_test, model_name):
    """Train and evaluate a model"""
    print(f"\nTraining {model_name}...")
    
    # Train the model
    model.fit(X_train, y_train)
    
    # Make predictions
    y_pred = model.predict(X_test)
    
    # Calculate metrics
    acc = accuracy_score(y_test, y_pred)
    f1 = f1_score(y_test, y_pred, average='weighted')
    report = classification_report(y_test, y_pred)
    
    print(f"\nModel: {model_name}")
    print("Accuracy:", acc)
    print("F1 Score:", f1)
    print("Classification Report:\n", report)
    
    return model, acc, f1

def main():
    """Main function to train and evaluate new hybrid models"""
    # Create results directory if it doesn't exist
    os.makedirs("results/new_hybrid", exist_ok=True)
    
    # Load preprocessed and balanced data
    print("Loading data...")
    df = pd.read_excel("data/balanced_sheet1.xlsx")
    
    # Separate features and targets
    X = df.drop(columns=["Injury Severity", "Injury Location"])
    y_severity = df["Injury Severity"]
    y_location = df["Injury Location"]
    
    # Encode Injury Location
    location_encoder = LabelEncoder()
    y_location_encoded = location_encoder.fit_transform(y_location)
    y_location = y_location_encoded
    
    # Split datasets
    X_train_s, X_test_s, y_train_s, y_test_s = train_test_split(X, y_severity, test_size=0.2, random_state=42)
    X_train_l, X_test_l, y_train_l, y_test_l = train_test_split(X, y_location, test_size=0.2, random_state=42)
    
    # Define new ensembles
    new_ensembles = {
        "Ensemble 6 (AdaBoost+GB+ExtraTrees)": create_ensemble_6(),
        "Ensemble 7 (Blending+MetaLearning)": create_ensemble_7(),
        "Ensemble 8 (WeightedAverage)": create_ensemble_8()
    }
    
    # Train and evaluate for Severity
    print("\n===== Training New Hybrid Models for Injury Severity =====")
    results_severity = []
    for name, model in new_ensembles.items():
        model, acc, f1 = train_and_evaluate(model, X_train_s, X_test_s, y_train_s, y_test_s, name)
        results_severity.append((name, acc, f1))
        joblib.dump(model, f"results/new_hybrid/{name.replace(' ', '_').lower()}_severity_model.pkl")
    
    # Train and evaluate for Location
    print("\n===== Training New Hybrid Models for Injury Location =====")
    results_location = []
    for name, model in new_ensembles.items():
        model, acc, f1 = train_and_evaluate(model, X_train_l, X_test_l, y_train_l, y_test_l, name)
        results_location.append((name, acc, f1))
        joblib.dump(model, f"results/new_hybrid/{name.replace(' ', '_').lower()}_location_model.pkl")
    
    # Create and save leaderboards
    results_df_s = pd.DataFrame(results_severity, columns=["Model", "Accuracy", "F1 Score"])
    results_df_l = pd.DataFrame(results_location, columns=["Model", "Accuracy", "F1 Score"])
    
    results_df_s = results_df_s.sort_values(by="F1 Score", ascending=False)
    results_df_l = results_df_l.sort_values(by="F1 Score", ascending=False)
    
    print("\n--- New Hybrid Models Severity Leaderboard ---")
    print(results_df_s)
    print("\n--- New Hybrid Models Location Leaderboard ---")
    print(results_df_l)
    
    # Save leaderboards
    results_df_s.to_csv("results/new_hybrid/new_hybrid_severity_leaderboard.csv", index=False)
    results_df_l.to_csv("results/new_hybrid/new_hybrid_location_leaderboard.csv", index=False)
    
    print("\nNew hybrid models training completed!")
    print("Models saved in results/new_hybrid/")

if __name__ == "__main__":
    main()
